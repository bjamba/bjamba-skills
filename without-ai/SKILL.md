---
name: without-ai
description: Plan an AI-free (or AI-trending-to-zero) infrastructure for a prompt the user would normally hand to an LLM — local-hardware, solo-maintainable, AI used only as a one-time bootstrap. Produces an overview with architecture diagrams, a phased roadmap, an "AI fork vs without-ai fork" comparison, and handoff packets for /teach-me, /productionize-me, and /hacky-hours. Optionally scaffolds the repo. Use when the user asks "how do I build this without an LLM", "how do I make this self-hostable", "design infrastructure for this", or is about to repeatedly throw the same kind of question at a model. Triggers on /without-ai, "minimize AI dependency", "trend AI to zero", "build me an editor/engine/DSL for X", or whenever a prompt implicates generating assets (art, copy, rules, layouts) that an LLM would one-shot but a real product needs a tool to produce consistently.
---
